...
from .models import Product, OrderDetail, User, Reviews
...
def AddReview(request, my_id):
    if request.method == 'POST':
        ...
        if text and rating:
            review = Reviews(text=text, rating=rating, name=name, item=item)
            review.save()
        else: reverse("myapp:index"),
